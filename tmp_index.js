...
async function forwardToWebhook(msg, phoneId, groupCache) {
    ...
    const fromNum = isGroup ? (msg.author || '').replace(/@\S+/, '') : jid.replace(/@\S+/, '');
    // Log to DB
    try {
    ...
    try {
        ...
        // Download media if present and save to uploads/
        let mediaUrl = null;
        if (msg.hasMedia) {
            try {
                ...
                if (mediaData && mediaData.data) {
                    ...
                    fs.writeFileSync(fpath, Buffer.from(mediaData.data, 'base64'));
                    const baseUrl = process.env.BASE_URL || 'https://integrasi-wa.jodyaryono.id';
                    mediaUrl = `${baseUrl}/uploads/${fname}`;
                }
            } catch (me) { console.error('[Media][' + phoneId + ']', me.message); }
        }
        const payload = {
            phone_id: phoneId, message_id: msg.id?._serialized, message: textContent,
            type: contentType?.replace('Message', '') || 'text',
            timestamp: msg.timestamp || Math.floor(Date.now() / 1000),
            sender: fromNum, sender_name: pushName, from: jid.replace(/@\S+/, ''),
            pushname: pushName,
